...
// ── PatrolSuggestion ──────────────────────────────────────────────────────
function SuggestPatrolForm({ incident, onSubmit, onCancel }) {
  ...
  const inputStyle = {
    ...
    padding: 12, fontSize: 14, color: colors.ink, backgroundColor: colors.surfaceAlt,
    marginBottom: 10,
  };
  ...
  return (
    <View style={{ padding: spacing.pad, backgroundColor: colors.accentSoft, borderRadius: radius.card, gap: 10 }}>
      ...
      <Text style={{ fontSize: 13, color: colors.inkSoft, lineHeight: 18, marginBottom: 4 }}>
        Describe when and where people should gather. No GPS — use landmarks or street descriptions.
      </Text>

      <TextInput
        value={time}
        onChangeText={setTime}
        placeholder="When? (e.g. Tonight at 8pm, In 30 min)"
      ...
      <TextInput
        ...
        placeholder="Where to meet? (e.g. Corner of Elm & 4th, near the red door)"
        placeholderTextColor={colors.inkFaint}
      ...
      <TextInput
        ...
        placeholder="Comment (optional) — what's the plan?"
        placeholderTextColor={colors.inkFaint}
        style={[inputStyle, { minHeight: 60, textAlignVertical: 'top' }]}
        multiline
      />
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        ...
        <Btn
          label="Post Patrol"
          icon="navigation"
          onPress={() => onSubmit({ time, gather, comment })}
          disabled={!canSubmit}
          style={{ flex: 2 }}
        />
      </View>
...
export default function IncidentDetailScreen({ route, navigation }) {
  ...
  const C = CATEGORIES[incident.cat];
  const catColorMap = {
  ...
  const catFg = catColorMap[incident.cat] || colors.inkFaint;
  const zoneName = zone ? (zone.name.split(' — ')[1] || zone.name) : '';
  const canSuggestPatrol = incident.confirms >= 3 && incident.status !== 'resolved';
  ...
  return (
    ...
    >
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: spacing.pad, gap: 16 }}>
        ...
        <Card style={{ padding: spacing.pad, gap: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <Text style={{ fontSize: 12, color: colors.inkFaint }}>
              {incident.confirms}/3 confirmations
            </Text>
          </View>

          <Text style={{ fontSize: 14.5, color: colors.inkSoft, lineHeight: 21 }}>
            {incident.note}
          </Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 4, borderTopWidth: 1, borderTopColor: colors.lineSoft }}>
            ...
            <Text style={{ fontSize: 12, color: colors.inkFaint, flex: 1 }}>
              Reported by {incident.reporter} · Trust {incident.reporterTrust}
            </Text>
          </View>
        </Card>
        ...
        {!incident.confirmedByMe && incident.status !== 'resolved' && (
          <Card style={{ padding: spacing.pad }}>
            ...
            <Text style={{ fontSize: 13, color: colors.inkSoft, lineHeight: 18, marginBottom: 12 }}>
              If you've witnessed this incident or can verify it's real, tap to confirm. Three community confirmations mark it as Verified.
            </Text>
        ...
        {/* Response / Patrols */}
        <View>
          <Text style={{ fontSize: 17, fontWeight: '800', color: colors.ink, marginBottom: 10 }}>
            Community Response
          </Text>
            ...
            <Card style={{ padding: spacing.pad, marginBottom: 10 }}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
                <View style={{
                  ...
                  backgroundColor: colors.lineSoft, alignItems: 'center', justifyContent: 'center',
                }}>
                  <Icon name="info" size={16} color={colors.inkFaint} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 13.5, fontWeight: '700', color: colors.ink, marginBottom: 3 }}>
                    Patrols unlock at 3 confirmations
                  </Text>
                  <Text style={{ fontSize: 12.5, color: colors.inkSoft, lineHeight: 17 }}>
                    Once this incident is confirmed by 3 community members, anyone can suggest a coordinated patrol walkthrough.
                  </Text>
                </View>
              </View>
            </Card>
          ...
          {canSuggestPatrol && !showPatrolForm && (
            <Btn
              label="Suggest a patrol"
              icon="navigation"
              variant="outline"
              onPress={() => setShowPatrolForm(true)}
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: p.goingByMe ? colors.accentSoft : colors.accent,
                    borderWidth: 1.5,
                    borderColor: p.goingByMe ? colors.accent : 'transparent',
                  }}
                >
                  <Text style={{
                    fontSize: 12, fontWeight: '700',
                    color: p.goingByMe ? colors.accent : colors.onAccent,
                  }}>
